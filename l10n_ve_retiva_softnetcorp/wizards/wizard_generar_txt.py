...
class BsoftContratoReport2(models.TransientModel):
    ...
    def action_generate_txt(self):
        ...
        with open("/usr/lib/python3/dist-packages/odoo/extra-addons/l10n_ve_retiva_softnetcorp/wizards/txt_generacion.txt", "w") as file:
        ...
        self.write({'file_data': base64.encodestring(open("/usr/lib/python3/dist-packages/odoo/extra-addons/l10n_ve_retiva_softnetcorp/wizards/txt_generacion.txt", "rb").read()),
                    'file_name': "Retenciones de IVA desde %s hasta %s.txt"%(self.date_from,self.date_to),
                    })

        return self.show_view('Arcivo Generado', self._name, 'l10n_ve_retiva_softnetcorp.snc_wizard_retencioniva_form_view', self.id)
